...
const base=[
  555,300,430,200,433,678,455,321,333,400,
  787,322,720,480,333,222,111,432,600,389,
  438,672,936,436,544,567,666,222,367,500,
]
...
export default function Generality() {
    ...
    useEffect(() => {
      AsyncStorage.getItem('input_caloires')
      .then(res=>setFoodCal(JSON.parse(res)))
      console.log("s");
    }, [])
    ...
    <ScrollView style={styles.contenPage} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.chart}>
        <View style={styles.chartDe}>
          ...
          <Text style={styles.chartData}>{dataChart.data.datasets[0].data[dataChart.data.datasets[0].data.length-1]} <Text style={styles.chartUnit}>cal</Text> </Text>
          <Text></Text>
            <Picker
      ...
      <View style={styles.cC}>
        <Text style={[styles.titleC,{color:'#365EE9',marginBottom:10}]}>Steps counter</Text>
        <View style={styles.cC1}>
          <Circle size={80} showsText={true} style={{marginLeft:10}} progress={0.4} />
      ...
      <View style={styles.cC}>
        ...
        <View style={styles.cCH}>
          <SvgIcon.Moon2 width="20" height="20"/>
          <Text style={[styles.titleC,{color:'#FF9C8A'}]}>Sleep</Text>
          <View>
          </View>
        </View>
        <Text style={styles.titleM}>8 <Text style={styles.titleU}>h</Text> 55 <Text style={styles.titleU}>min</Text> </Text>
        </View>
      </View>
      <View style={styles.cC}>
       <View style={{paddingHorizontal:20,paddingVertical:10}}>
        <View style={styles.cCH}>
          <>
          <SvgIcon.Heart width="20" height="20"/>
          <Text style={[styles.titleC,{color:'#FF235B'}]}>Heart beat</Text>
          </>
        </View>
        <Text style={styles.titleM}>90 <Text style={styles.titleU}>BPM</Text></Text>
        </View>
      </View>
      <View style={styles.cC}>
      <View style={{paddingHorizontal:20,paddingVertical:10}}>
        <View style={styles.cCH}>
          <>
          <SvgIcon.Apple width="20" height="20"/>
          <Text style={[styles.titleC,{color:'#E83C00'}]}>Nutrition today</Text>
          </>
        </View>
        <Text style={styles.titleM}>{foodCal.value} <Text style={styles.titleU}>cal</Text></Text>
        </View>
      </View>
    </ScrollView>
  );
