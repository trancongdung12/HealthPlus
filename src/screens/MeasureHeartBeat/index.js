...
const MeasureHeartBeat = () => {
  const [output, setOutput] = useState(0)
   ...
   const finishAnimate= (bpm)=>{
    Torch.switchState(false); 
     AsyncStorage.setItem('heart_beat',JSON.stringify({value: bpm}))
     .then(res=>{
      setOutput(bpm)})
   }
  ...
  return (
    <View style={styles.container}>
            ...
            (fill)=>(
              ...
              <View style={styles.Hc}>
                ...
                <Text style={styles.out}>{
                 fill>90&&bpm
                } <Text style={styles.outU}>BPM</Text> </Text>
                <TouchableOpacity onPress={()=>forceUpdate()} style={styles.btn}>
              ...
              <View style={styles.Hc}>
                 <SvgIcon.Heart size={32}/>
                 <Text style={styles.out}>00<Text style={styles.outU}>BPM</Text> </Text>
                <TouchableOpacity onPress={()=>startAnimate()} style={styles.btn}>
                   <View style={styles.ic}>
                   <SvgIcon.Start/>
                   </View>
      ...
      <Text style={styles.tOut}>{output} <Text style={styles.tOutU}>BPM</Text> </Text>
      <SVG.HeartRateOut/>
